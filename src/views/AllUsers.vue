...
// Convert formatted tag back to API format (e.g., "Filmmaker" -> "filmmaker_1485")
// We'll need to search through all tags to find the matching one
const findTagKey = (formattedTag: string, tagsData: any): string | null => {
  for (const tagKey in tagsData) {
    const formatted = formatTagName(tagKey);
    if (formatted.toLowerCase() === formattedTag.toLowerCase()) {
      return tagKey;
    }
  }
  return null;
...
// Format tag name (same as in ExploreCategories)
...
const fetchAllUsers = async () => {
  try {
    ...
    let foundTagData: any = null;
    
    // Try to find matching tag
    for (const key in tagsData) {
      const formatted = formatTagName(key);
      if (formatted.toLowerCase() === category.value.toLowerCase()) {
        tagKey = key;
        foundTagData = tagsData[key];
        break;
      }
    }
    ...
    if (!tagKey || !foundTagData) {
      console.error('Tag not found. Category:', category.value);
      const availableTags = Object.keys(tagsData).map(key => formatTagName(key));
      console.log('Available formatted tags:', availableTags);
      error.value = `Tag "${category.value}" not found. Available tags: ${availableTags.join(', ')}`;
    ...
    if (foundTagData && foundTagData.length > 0 && foundTagData[0].users) {
      ...
      tagName.value = formatTagName(tagKey);
...
<template>
  <main class="min-h-screen bg-white text-[#111111] max-w-5xl mx-auto px-4 sm:px-6 lg:px-10 pb-10">
    ...
    <!-- Title -->
    <section class="mt-6">
      <h1 class="text-2xl font-bold mb-2">{{ tagName || category || 'All Users' }}</h1>
    ...
    <section v-else class="mt-8">
      <div v-if="users.length === 0" class="text-center py-12">
        ...
        <p class="text-sm text-gray-500">Users array length: {{ users.length }}</p>
      </div>
